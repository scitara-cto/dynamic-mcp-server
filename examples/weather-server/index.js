...
const weatherHandlerPackage = {
  ...
  handler: async (args, context, config, progress = () => null) => {
    ...
    if (config.action === "explain-forecast") {
      ...
      let explanation = `# Weather Forecast Explanation for ${
        location || "Your Location"
      }\n\n`;
      explanation += "## Understanding Weather Data\n\n";
      explanation +=
        "Weather forecasts provide several key pieces of information:\n\n";
      explanation += "- **Temperature**: Current and feels-like temperatures\n";
      explanation +=
        "- **Humidity**: Moisture content in the air (affects comfort)\n";
      explanation +=
        "- **Wind**: Speed and direction (affects temperature perception)\n";
      explanation +=
        "- **Pressure**: Atmospheric pressure (indicates weather changes)\n";
      explanation +=
        "- **Visibility**: How far you can see (affected by fog, rain, etc.)\n";
      explanation +=
        "- **UV Index**: Strength of ultraviolet radiation from the sun\n\n";
      ...
      if (includeAdvice === "true" || includeAdvice === true) {
        ...
        explanation +=
          "- **Above 25°C (77°F)**: Light, breathable clothing, shorts, t-shirts\n";
        explanation +=
          "- **15-25°C (59-77°F)**: Light layers, long pants, light jacket\n";
        explanation +=
          "- **5-15°C (41-59°F)**: Warm layers, jacket, closed shoes\n";
        explanation +=
          "- **Below 5°C (41°F)**: Heavy coat, warm layers, gloves, hat\n\n";

        explanation += "### Activity Suggestions:\n";
        explanation += "- **Sunny Weather**: Outdoor sports, hiking, picnics\n";
        explanation +=
          "- **Rainy Weather**: Indoor activities, museums, shopping\n";
        explanation +=
          "- **Windy Weather**: Avoid outdoor activities with loose objects\n";
        explanation +=
          "- **Extreme Weather**: Stay indoors, avoid unnecessary travel\n\n";
      }

      explanation += "## Tips for Weather Planning\n\n";
      explanation +=
        "1. Check the forecast regularly as conditions can change\n";
      explanation +=
        "2. Pay attention to 'feels like' temperature for comfort\n";
      ...
      return {
        description: `Weather forecast explanation for ${
          location || "your location"
        }`,
        messages: [
          {
            ...
            content: {
              type: "text",
              text: `Can you explain the weather forecast for ${
    ...
    if (config.action === "api-guide") {
      ...
      guide +=
        "4. **API Keys**: Keep your API keys secure and never expose them\n";
      guide +=
        "5. **Units**: Specify units (metric/imperial) for consistent data\n\n";
        ...
        messages: [
          {
            ...
            content: {
              type: "text",
              text: `What safety tips should I know for ${
                weatherCondition || "various weather conditions"
    ...
    progress(0, 100, "Starting web request...");
    ...
    // Set up headers - accept multiple content types
    const headers = {
      "Accept": "text/html,application/xhtml+xml,application/xml,application/json,text/plain,*/*",
      "User-Agent": "Mozilla/5.0 (compatible; MCP-Weather-Server/1.0)",
    };
    ...
    progress(80, 100, "Processing response...");
    ...
    return {
      ...
      message: "Web request successful"
    };
  },
};
